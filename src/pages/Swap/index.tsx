...
import { mapToken, reverseMapToken, reverseMapTokenAmount } from '../../polywrap-utils'
...
import { Uni_Trade, Uni_TradeTypeEnum } from '../../wrap'
...
const formatApproveTokenTxnSubmittedEventProperties = (approvalOptimizedTrade: Uni_Trade | undefined) => {
  if (!approvalOptimizedTrade) return {}
  return {
    chain_id: approvalOptimizedTrade.inputAmount.token.chainId,
    token_symbol: approvalOptimizedTrade.inputAmount.token.currency.symbol,
    token_address: getTokenAddress(reverseMapToken(approvalOptimizedTrade.inputAmount.token)!),
  }
}
...
export default function Swap() {
  ...
  const inputValue = showWrap ? parsedAmount : reverseMapTokenAmount(trade?.inputAmount)!
  const outputValue = showWrap ? parsedAmount : reverseMapTokenAmount(trade?.outputAmount)!
  ...
  const [{ showConfirm, tradeToConfirm, swapErrorMessage, attemptingTxn, txHash }, setSwapState] = useState<{
    showConfirm: boolean
    tradeToConfirm: Uni_Trade | undefined
  ...
  const handleApprove = useCallback(async () => {
    if (signatureState === UseERC20PermitState.NOT_SIGNED && gatherPermitSignature) {
      ...
      sendEvent({
        category: 'Swap',
        action: 'Approve',
        label: [approvalOptimizedTradeString, trade?.inputAmount?.token.currency.symbol].join('/'),
      })
    }
  }, [
    signatureState,
  ...
  const polywrapCurrencies = useMemo(
    () => ({
      INPUT: currencies.INPUT ? mapToken(currencies.INPUT) : undefined,
      OUTPUT: currencies.OUTPUT ? mapToken(currencies.OUTPUT) : undefined,
    }),
    [currencies]
  )
  ...
  return (
    <Trace page={PageName.SWAP_PAGE} shouldLogImpression>
      <>
        ...
        />
        <Flex flexWrap="wrap" sx={{ justifyContent: 'center' }} width={1}>
          <AppBody>
                    ...
                    <AutoRow style={{ flexWrap: 'nowrap', width: '100%' }}>
                      <AutoColumn style={{ width: '100%' }} gap="12px">
                        <TraceEvent
                          ...
                          properties={formatApproveTokenTxnSubmittedEventProperties(trade)}
          ...
          <Codeblock
            currencies={polywrapCurrencies}
            input={trade?.inputAmount.amount}
            output={trade?.outputAmount.amount}
            tradeType={(trade?.tradeType ?? Uni_TradeTypeEnum.EXACT_INPUT) as Uni_TradeTypeEnum}
