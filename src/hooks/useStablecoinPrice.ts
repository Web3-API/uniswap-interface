...
export default function useStablecoinPrice(currency?: Currency): Price<Currency, Token> | undefined {
  ...
  // todo: which to use?
  const trade = useClientSideV3Trade(TradeType.EXACT_OUTPUT, amountOut, currency)
  ...
  const price = useMemo(() => {
    ...
    if (trade.trade) {
      const { numerator, denominator } = trade.trade.swaps[0].route.midPrice
      return new Price(currency, stablecoin, denominator, numerator)
    }

    return undefined
  }, [...currencyDepsSDK(currency), stablecoin, trade.trade])
